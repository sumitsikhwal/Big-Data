...
val calllog = sc.textFile("/sks130730/tip.txt")
...
{
 val cluster=callsByGoup(i)._2.iterator.toList.map(x => x.toList) 
  for(li <- cluster)
   {str=   i + "," +li(0)
   list +=str }
}
...
sc.parallelize(list,1).saveAsTextFile("/sks130730/ProjectOutput/calldur.txt")
...
sc.parallelize(list1,1).saveAsTextFile("/sks130730/ProjectOutput/calldur1.txt")